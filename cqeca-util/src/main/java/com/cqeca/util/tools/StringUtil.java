...
public class StringUtil {
	...
	public static boolean isContainStr(String str,List<String> list) {
		if(StringUtil.isBlank(str) || null == list || list.size() == 0) {
			return false;
		}
	 ...
	 * 加密字符串，以s代替
	 * @param str		要加密的原始字符串
	 * @param index		开始加密位置：同数组下标，加密包含该位置
	 * @param lenght	加密长度
	 * @param s			替换字符
	 * @return
	 * @author 	奚渭雄
	 * @date 	2014-10-27下午3:32:01
	 */
	public static String encryption(String str, int index, int lenght, char s) {
